...
export async function MyVideosView({ kidId }: { kidId: string }) {
  const { parent, kid } = await requireKidContext(kidId);
  const [allApproved, channels, pending] = await Promise.all([
  ...
  const videos = allApproved.filter((v) => v.visibleDays.includes(dow));
